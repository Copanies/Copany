...
export abstract class GenericDataManager<T, TItem = T> {
  ...
  /**
   * 智能设置单个数据项（比较时间戳，只保留最新数据）
   */
  smartSetItem(
    key: string,
    item: TItem & { updated_at?: string | null }
  ): void {
    if (
      !this.itemOps?.findItem ||
      !this.itemOps?.updateItem ||
      !this.itemOps?.addItem
    ) {
      console.warn(
        `[${this.config.managerName}] smartSetItem operations not fully configured`
      );
      return;
    }
    ...
    if (!data) {
      console.log(
        `[${this.config.managerName}] 🆕 No cached data, adding new item`
      );
    ...
    // 尝试查找现有项
    const itemId = (item as { id?: string | number })?.id?.toString();
    if (!itemId) {
      console.warn(
        `[${this.config.managerName}] ⚠️ Item has no ID, cannot smart set`
      );
      return;
    }

    const existingItem = this.itemOps.findItem(data, itemId) as {
      updated_at?: string | null;
    };
    if (!existingItem) {
      console.log(
        `[${this.config.managerName}] 🆕 Item not found, adding new item: ${itemId}`
      );
      this.addItem(key, item);
      return;
    }
    ...
    // 比较时间戳
    const newTimestamp = item.updated_at
      ? new Date(item.updated_at).getTime()
      : Date.now();
    const existingTimestamp = existingItem.updated_at
      ? new Date(existingItem.updated_at).getTime()
      : 0;

    if (newTimestamp >= existingTimestamp) {
      console.log(
        `[${this.config.managerName}] 🔄 Updating with newer data: ${itemId}`
      );
      this.updateItem(key, itemId, item);
    } else {
      console.log(
        `[${this.config.managerName}] 🚫 Skipping older data: ${itemId}`
      );
    }
  }
