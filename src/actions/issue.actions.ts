...
// Comment actions
export async function getIssueCommentsAction(issueId: string) {
  return await IssueService.getIssueComments(issueId);
}
...
export async function createIssueCommentAction(
  ...
  return await IssueService.createIssueComment({
    issue_id: issueId,
    content,
    created_by: user.id,
...
export async function updateIssueCommentAction(
  ...
  return await IssueService.updateIssueComment(commentId, content);
}

export async function deleteIssueCommentAction(commentId: string) {
  const user = await getCurrentUser();
  if (!user) {
    throw new Error("User not found");
  }
  return await IssueService.deleteIssueComment(commentId);
}
