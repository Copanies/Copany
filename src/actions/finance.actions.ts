...
export async function regenerateDistributesForCurrentMonthAction(copanyId: string) {
  ...
  const { data: txs, error: txErr } = await admin
    .from("transactions")
    .select("*")
    .eq("copany_id", copanyIdNumeric as any)
    .eq("status", "confirmed")
  ...
  (completedIssues || []).forEach((issue: any) => {
    ...
    if ([IssueLevel.level_C, IssueLevel.level_B, IssueLevel.level_A, IssueLevel.level_S].includes(level)) {
  ...
  const contributorsWithScores = (contributors || []).map((c: any) => ({
    ...c,
  ...
  if (netIncome <= 0) {
    ...
    const rows = contributorsWithScores.map((c: any) => ({
      copany_id: copanyId,
      to_user: c.user_id,
    ...
    const { error: insErr } = await admin.from("distribute").insert(rows as any[]);
